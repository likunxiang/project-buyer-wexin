...
module.exports = function (object) {
    ...
    let core = this.core;
    let access_token = this.core.getStorageSync(this.const.ACCESS_TOKEN);//'_cMXYy7ukzBaxPY9qvj2ag6_vi-wAW1P'
    let formIdList = this.core.getStorageSync(this.const.FORM_ID_LIST);
    if (access_token) {
        object.header.access_token = access_token;
        object.data.access_token = access_token;
    }
    // object.data._version = this._version;
    object.data._platform = this.platform;
    if(wx.getStorageSync('_mchInfo')){
        object.data.mch_id = wx.getStorageSync('_mchInfo').id;
        object.header['mch-id'] = object.data.mch_id;
    }
    if (wx.getStorageSync('promoter')){
        object.data.promoter = wx.getStorageSync('promoter').id
    } else {
        object.data.promoter = ''
    }
    object.header.promoter = object.data.promoter;
    object.header.version = this.envInfo.version;
