...
module.exports = {
	...
	goodsOnLoad () {
		...
		if (wx.getStorageSync('service_tips')) {
			self.setData({
				is_show_again: false
			})
		} else {
	...
	showAgain() {
		...
		wx.setStorageSync('service_tips', true)
	},
	//收藏
		...
		wx.getStorage({
			...
			success(user) {
				wx.getStorage({
					...
					success(mch) {
						wx.navigateTo({
							url: '/pages/web/web?type=2&user_id=' + user.data.id + '&mch_id=' + mch.data.id + '&gs_id=' + self.data.goods
								.id,
						})
					},
				})
	...
	addCart: util.throttle(function() {
		...
		if (self.data.btn) {
			...
			if (self.data.pageType == "PROUPGOODS") {
	...
	submit: function(type) {
		...
		if (self.data.miaosha_data && self.data.miaosha_data.rest_num > 0 && self.data.form.number > self.data.miaosha_data
			.rest_num) {
			getApp().core.showToast({
