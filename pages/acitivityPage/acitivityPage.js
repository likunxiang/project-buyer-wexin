...
Page({
	...
	openCart (e) {
		...
		var goods = this.data.miaoshaList[index]
		this.setData({
	...
	getScrollList() {
		getApp().request({
			url: getApp().api.default.ms_record,
			success: (res) => {
				if (res.code == 0) {
					this.setData({
						scrollList: res.data
	...
	getMore() {
		...
		getApp().core.pading(self, url, data, function(res) {
			...
			var nowGoodList = self.data.miaoshaList.concat(newGoodList)
			self.setData({
